...
def fetch_real_test_data():
    """使用data_fetching.py中的函数获取真实测试数据"""
    try:
        # 导入数据获取函数
        from mrk_data.utils.from_vnpy import Exchange
        from src.data.data_fetching import fetch_tick_data_range

        logger.info("正在获取真实测试数据...")

        # 使用配置文件中的参数或默认参数
        instrument_id = "SSE.10009482"
        exchange = Exchange.SSE
        start_date = 20250801  # 使用一个已知有数据的日期
        end_date = 20250801

        logger.info(
            f"获取数据参数: instrument_id={instrument_id}, exchange={exchange}, start_date={start_date}, end_date={end_date}"
        )
        ...
        if df is not None and not df.empty:
            logger.info(f"成功获取真实数据: {df.shape}")
            return df
        else:
            logger.warning("真实数据获取失败或为空，使用模拟数据")
            return None

    except Exception as e:
        logger.error(f"获取真实数据时出错: {e}")
        logger.info("将使用模拟数据作为备选")
        return None


def load_fallback_data(filename="test_orderbook_data_accurate.csv"):
    """加载备选测试数据"""
    try:
        df = pd.read_csv(filename)
        df["datetime"] = pd.to_datetime(df["datetime"])
        logger.info(f"成功加载备选测试数据: {df.shape}")
        return df
    except FileNotFoundError:
        logger.error(f"备选测试数据文件 {filename} 未找到")
        return None


def main():
    """主函数 - 运行完整的模式识别研究流程"""
    logger.info("开始模式识别研究测试（使用真实数据）")

    # 尝试获取真实数据
    df = fetch_real_test_data()

    # 如果真实数据获取失败，使用备选数据
    if df is None:
        logger.info("使用备选测试数据...")
        df = load_fallback_data()
        if df is None:
            logger.error("无法获取任何测试数据，退出")
            return
    ...
    # 保存结果
    output_file = "pattern_recognition_results_real_data.csv"
